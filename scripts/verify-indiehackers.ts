// One-off verification script for the IndieHackers source integration —
// not part of the shipped app. Run with:
//   npx tsx -r dotenv/config scripts/verify-indiehackers.ts
...
async function main() {
  ...
  const existing = await prisma.source.findUnique({
    where: { productId_name: { productId: product.id, name: "IndieHackers" } },
  });
  if (existing) {
    ...
    console.log(`Re-enabled existing IndieHackers source ${existing.id}`);
  } else {
    ...
    console.log(`Created IndieHackers source ${created.id}`);
  }
